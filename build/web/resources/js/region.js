...
//var path = '${pageContext.servletContext.contextPath}';//context de l'application
        ...
        table = $('#tableRegion').DataTable(
        {
         ...
         $('#supprimerRegion').click(function(){
           ...
           for(var i =0; i<tableauData.length; i++)
           {
               ...
               {
                   idRegionSupprimer.push(tableauIntermediaire[3]);
               }
              
           }
           table.rows('.selected').remove().draw();
           if(idRegionSupprimer.length>0)
           {
                supprimerRegion(idRegionSupprimer);
           }else{
        ...
        $('#tableRegion tbody').on('click', 'input[type="checkbox"]', function(e)
        {
           ...
           */
           if(table.rows('.selected').data().length===1)
           {
               ...
               {
                   document.getElementById("nomRegionUpdate").setAttribute('value',tableauResultat[1]);
               }
                
                if(tableauResultat[3]!==null)
                {
                    document.getElementById("idRegionUpdate").setAttribute('value',tableauResultat[3]); 
                }   
    ...
    function addRegion() 
    {
        //$('#linkupdate').hide();
        var region = {};
        region.nomRe =document.querySelector("#nomRegion").value;
        region.id=document.querySelector("#idRegion").value;
        var url =''+path+'/ControllerCentralRegion?action=add';
        
        $('input[type="text"]').each(function(){
            if($(this).val()!=='' && $(this).attr('id')==='nomRegion')
            {
                region.nomRe=$(this).val();
        ...
        xhr.onloadend = function () 
        {
            ...
            var regionR = JSON.parse(this.response);
            if(typeof regionR !='undefined')//== "undefined"
            {
                if(regionR.id!==null)
                {
                    table.row.add( [ '', region.nomRe, regionR.dateCreatRegion, regionR.id]).draw();
                    $('input[type="text"]').each(function()
                    {
                        if($(this).val()!=='' && $(this).attr('id')==='nomRegion')
                        {
        ...
        xhr.send(JSON.stringify(region));

        return false;
    }
    
    function supprimerRegion(tableauId)
    {
        var url =''+path+'/ControllerCentralRegion?action=delete';
        var xhr = new XMLHttpRequest();
        xhr.open('POST', url, true);
    ...
    function updateRegion() 
    {
        ...
        var url =''+path+'/ControllerCentralRegion?action=update'; 
        $('input[type="text"]').each(function(){
            if($(this).val()!=='' && $(this).attr('id')==='nomRegionUpdate')
            {
                region.nomRe=$(this).val();
                //console.log('value ' + $(this).val() +' id ' + $(this).attr('id') +' index ' + index);
            }
            
            if($(this).val()!=='' && $(this).attr('id')==='idRegionUpdate')
            {
                region.id=$(this).val();
            }
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', url, true);
        console.log("id go " + region.id);

        xhr.onloadend = function () 
        {
            var regionR = JSON.parse(this.response);
            console.log("update " + JSON.stringify(regionR));
            if(typeof regionR !='undefined')//== "undefined"
            {
                if(regionR.id!==null)
                {
